/**
 * Simple e-commerce cart manager using localStorage.
 */

export const getCart = () => {
  try {
    return JSON.parse(localStorage.getItem("byk_cart")) || [];
  } catch (e) {
    return [];
  }
};
...
export const saveCart = (cart) => {
  localStorage.setItem("byk_cart", JSON.stringify(cart));
...
export const clearCart = () => {
  localStorage.removeItem("byk_cart");
  window.dispatchEvent(new Event("cart_updated"));
};
