export const ThreeDayWellnessRetreatData = {
  heroSection: {
      colors: {
        cream: "#F5F8F0",
        goldLight: "#c6d7ba",
        navy: "#1A2E10",
        sage: "#4A7A30",
        overlay: "linear-gradient(180deg, rgba(26,46,16,0.50), rgba(26,46,16,0.88))",
        white: "#ffffff",
        textLight: "rgba(255,255,255,0.90)",
        textFade: "rgba(255,255,255,0.80)",
      },
  
      hero: {
        location: "Ubud, Bali · 3-Day Yoga & Wellness Retreat",
        title: "3-Day Yoga and Wellness Retreat",
        highlight: "3-Day ",
        subtitle: "Yoga, meditation, massage & wellness with customized Panchakarma consultation",
        price: "$350",
        priceNote: "2 sharing room — includes stay, sattvic meals & wellness therapy",
        bgImage: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1600&q=80",
        certificateImage:
          "https://www.ombreathe.in/static/media/Yogalyaa_200_Hour_TTC_1st%20image.68041e974b4b6b7d159e.jpg",
        buttonText: "Book Now",
        url: "/contact",
      },
  
      gains: [
        "Personalized Panchakarma consultation with an experienced Ayurvedic Doctor",
        "Learn fundamental principles of yoga: asanas, pranayama, meditation & philosophy",
        "Panchakarma therapy to remove deep-rooted toxins and balance the three doshas",
        "Restores digestive fire, strengthens immunity, and increases mental clarity",
        "Daily yoga, meditation, and evening relaxation sessions",
      ],
  
      certificates: [
        { img: "/images/cirtificats/yoga.png", label: "3 Days" },
        { img: "/images/cirtificats/yoga.png", label: "Wellness" }
      ],
  
      content: {
        bottomText: "A compact and impactful wellness break designed for working professionals seeking renewal.",
      },
    },

  highlightsSection: {
    community: {
        colors: {
          white: "#ffffff",
          goldLight: "#7AAA5A",
          overlay: "linear-gradient(180deg, rgba(30,60,15,0.65), rgba(10,25,5,0.95))",
          textLight: "rgba(255,255,255,0.75)",
          textFade: "rgba(255,255,255,0.7)",
          glass: "rgba(255,255,255,0.08)",
          glassHover: "rgba(255,255,255,0.15)",
          border: "rgba(255,255,255,0.2)",
        },
    
        content: {
          bgImage: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1600&q=80",
          eyebrow: "Ombreathe Retreat",
          title: "Join our",
          highlight: "wellness retreat",
          subtitle:
            "A short yet meaningful break to experience the bliss of yoga and connect with your inner self, with personalized Panchakarma consultation.",
          stats: [
            { value: "3 Days", label: "Wellness Retreat", icon: "clock" },
            { value: "Panchakarma", label: "Ayurvedic Cleansing", icon: "leaf" },
            { value: "Ayurveda", label: "Personalized Doctor Consult", icon: "heart" },
            { value: "All Levels", label: "Welcome", icon: "globe" },
          ],
          bottomText1:
            "Perfect for working professionals tired of daily routine, longing for a short yet meaningful break.",
          bottomText2:
            "Each guest receives a detailed consultation with an Ayurvedic Doctor and a customized wellness plan based on their Prakriti.",
        },
      },
    promo: {
        colors: {
          navy: "#1A2E10",
          violet: "#4A7A30",
          white: "#ffffff",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
          overlay: "linear-gradient(to top, rgba(0,0,0,0.4), transparent)",
        },
    
        content: {
          eyebrow: "Bali Wellness Retreat",
          title: "Join our",
          highlight: "restorative",
          duration: "6-day",
          strongText: "Ubud",
          features: [
            {
              icon: "heart",
              title: "Shat Karma & Panchakarma",
              sub: "Deep yogic cleansing & Ayurvedic detox",
            },
            {
              icon: "feather",
              title: "Daily Yoga & Meditation",
              sub: "Hatha yoga, pranayama & Yoga Nidra",
            },
            {
              icon: "utensils",
              title: "Sattvic Detox Meals",
              sub: "Freshly cooked vegetarian & vegan food",
            },
            {
              icon: "sparkles",
              title: "Ayurvedic Massage",
              sub: "Wellness therapies & rejuvenation sessions",
            },
            {
              icon: "compass",
              title: "Spiritual Talks",
              sub: "Philosophy, yogic lifestyle & inner wisdom",
            },
          ],
          images: {
            main:
              "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1200&q=80",
            food:
              "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800&q=80",
            stay:
              "https://images.unsplash.com/photo-1540555700478-4be289fbecef?w=800&q=80",
          },
        },
      }

  },

  practiceSection: {
      colors: {
        navy: "#1A2E10",
        cream: "#F5F8F0",
        white: "#ffffff",
        shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
        overlay: "linear-gradient(to top, rgba(0,0,0,0.4), transparent)",
      },
  
      content: {
        eyebrow: "Retreat Practices",
        title: "Restore Through",
        highlight: "Yoga, Detox & Ayurveda",
        subtitle:
          "A thoughtfully designed combination of yogic cleansing, Ayurvedic healing, daily yoga and meditative practices for complete renewal.",
        practices: [
          {
            label: "Yoga & Breathwork",
            desc: "Morning Hatha yoga, pranayama and detox breathwork to cleanse the body, breath and mind each day",
            icon: "sun",
            img: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&q=80",
          },
          {
            label: "Shat Karma & Panchakarma",
            desc: "Traditional yogic cleansing processes — remove deep-rooted toxins, balance doshas and restore digestive fire",
            icon: "leaf",
            img: "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=800&q=80",
          },
          {
            label: "Ayurveda & Wellness Therapy",
            desc: "Customized Ayurvedic massage, rejuvenation therapy, Yoga Nidra and spiritual philosophy talks",
            icon: "heart",
            img: "https://images.unsplash.com/photo-1540555700478-4be289fbecef?w=800&q=80",
          },
        ],
        specialTitle: "Why Join",
        specialHighlight: "This Retreat",
        specials: [
          {
            title: "Perfect for Busy Professionals",
            desc: "A compact 6-day retreat designed for those with limited time who need deep renewal and rest",
            icon: "heart",
          },
          {
            title: "Traditional Detox Methods",
            desc: "Authentic Shat Karma and Panchakarma — powerful cleansing that goes beyond surface-level wellness",
            icon: "leaf",
          },
          {
            title: "All Levels Welcome",
            desc: "No prior yoga experience needed — travelers, beginners and seasoned practitioners all welcome",
            icon: "sparkles",
          },
        ],
      },
    },

  programDetailsSection: {
    curriculum: {
        colors: {
          navy: "#1A2E10",
          violet: "#4A7A30",
          sage: "#7AAA5A",
          cream: "#F5F8F0",
          white: "#ffffff",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
          gradientCard: "linear-gradient(135deg,#1A2E10,#4A7A30)",
          gradientCTA: "linear-gradient(135deg,#1A2E10,#2E5018)",
        },
    
        content: {
          eyebrow: "Retreat Program",
          title: "Your Complete",
          highlight: "Wellness Journey",
          subtitle:
            "A holistic 6-day program combining yogic cleansing, Ayurvedic healing, daily yoga, meditation and spiritual philosophy.",
    
          courses: [
            {
              title: "Hatha Yoga",
              icon: "sun",
              items: [
                "Morning yoga sessions",
                "Basic asana practice",
                "Alignment fundamentals",
                "Yoga for detox",
              ],
            },
            {
              title: "Pranayama & Detox",
              icon: "wind",
              items: [
                "Detox breathwork",
                "Shat Karma cleansing",
                "Nadi purification",
                "Energy activation",
              ],
            },
            {
              title: "Meditation",
              icon: "moon",
              items: [
                "Morning meditation",
                "Mindfulness practice",
                "Inner awareness",
                "Stress release",
              ],
            },
            {
              title: "Panchakarma",
              icon: "leaf",
              items: [
                "Deep toxin removal",
                "Dosha balancing",
                "Vata, Pitta & Kapha",
                "Tissue rejuvenation",
              ],
            },
            {
              title: "Ayurvedic Massage",
              icon: "heart",
              items: [
                "Rejuvenation therapy",
                "Wellness healing",
                "Body nourishment",
                "Deep relaxation",
              ],
            },
            {
              title: "Yoga Nidra",
              icon: "feather",
              items: [
                "Yoga Nidra sessions",
                "Deep rest practice",
                "Conscious relaxation",
                "Inner renewal",
              ],
            },
            {
              title: "Yogic Philosophy",
              icon: "book",
              items: [
                "Spiritual talks",
                "Yogic lifestyle",
                "Vedic wisdom",
                "Daily philosophy",
              ],
            },
            {
              title: "Wellness Therapies",
              icon: "sparkles",
              items: [
                "Customized healing",
                "Rejuvenation program",
                "Immunity strengthening",
                "Mental clarity",
              ],
            },
          ],
    
          phases: [
            {
              title: "Cleanse",
              days: "Days 1–2",
              desc: "Arrival, orientation, Shat Karma, Panchakarma & yoga foundation",
              color: "#2E5018",
            },
            {
              title: "Restore",
              days: "Days 3–4",
              desc: "Ayurvedic therapy, meditation, philosophy & deep relaxation",
              color: "#4A7A30",
            },
            {
              title: "Renew",
              days: "Days 5–6",
              desc: "Integration, Yoga Nidra, spiritual talks & inner renewal",
              color: "#7AAA5A",
            },
          ],
    
          cta: {
            title: "Book Your Wellness Retreat in Bali",
            desc:
              "Reset, detox and reconnect through 6 days of yoga, Ayurveda and deep inner renewal in sacred Ubud.",
            buttonText: "Book Your Retreat",
              url:"/contact",
          },
        },
      },
    schedule: {
        colors: {
          cream: "#F5F8F0",
          creamDark: "#DFF0D0",
          navy: "#1A2E10",
          violet: "#4A7A30",
          white: "#ffffff",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
          noteBg: "rgba(74,122,48,0.08)",
        },
    
        content: {
          eyebrow: "Daily Schedule",
          title: "A Day at",
          highlight: "Your Wellness Retreat",
          subtitle: "Each day balances yoga, wellness therapy, nourishing meals, and peaceful restoration.",
          image: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1200&q=80",
          morning: [
            { time: "06:00 AM", activity: "Yoga", icon: "sun" },
            { time: "07:30 AM", activity: "Meditation and pranayama, detox", icon: "wind" },
            { time: "09:00 AM", activity: "Breakfast", icon: "coffee" },
            { time: "11:00 AM", activity: "Rejuvenation and wellness therapy", icon: "heart" },
            { time: "01:00 PM", activity: "Lunch", icon: "utensils" },
          ],
          afternoon: [
            { time: "03:00 PM", activity: "Spiritual talk / Philosophy / Free time", icon: "book" },
            { time: "05:00 PM", activity: "Yoga / Yoga Nidra / Massage", icon: "feather" },
            { time: "06:30 PM", activity: "Dinner", icon: "utensils" },
          ],
          sections: {
            morningTitle: "Morning",
            afternoonTitle: "Afternoon & Evening",
            note: "Depending on the season and weather, the timetable and timings can be subject to change.",
          },
        },
      },
  },

  experienceSection: {
    food: {
        colors: {
          cream: "#F5F8F0",
          navy: "#1A2E10",
          white: "#ffffff",
          sage: "#4A7A30",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
        },
    
        content: {
          eyebrow: "Retreat Nourishment",
          title: "Detox & Nourish",
          highlight: "body & soul",
          subtitle:
            "Freshly cooked sattvic vegetarian meals 3 times daily — pure detox food to support your deep cleansing retreat.",
    
          meals: [
            {
              key: "breakfast",
              title: "Breakfast",
              icon: "sun",
              time: "09:00 – 10:00",
              img: "https://www.cookwithkushi.com/wp-content/uploads/2021/11/easy_vegetarian_indian_breakfast_recipes.jpg",
              desc: "Light and energizing sattvic breakfast after your morning yoga and detox practices.",
              items: [
                "Fresh tropical fruits",
                "Detox juices",
                "Herbal teas",
                "Light grain dishes",
              ],
            },
            {
              key: "lunch",
              title: "Lunch",
              icon: "coffee",
              time: "01:00 – 02:00",
              img: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800&q=80",
              desc: "Nourishing Ayurvedic vegetarian lunch to support your afternoon wellness therapies.",
              items: [
                "Detox rice bowls",
                "Fresh salads",
                "Vegetable curries",
                "Healing soups",
              ],
            },
            {
              key: "dinner",
              title: "Dinner",
              icon: "moon",
              time: "06:30 – 07:30",
              img: "https://images.unsplash.com/photo-1547592180-85f173990554?w=800&q=80",
              desc: "Light calming sattvic evening meals to support deep rest and overnight healing.",
              items: [
                "Lentil soups",
                "Steamed vegetables",
                "Light grain dishes",
                "Herbal teas",
              ],
            },
          ],
    
          dietary: [
            { icon: "leaf", label: "Vegetarian" },
            { icon: "feather", label: "Vegan" },
            { icon: "sparkles", label: "Gluten-Free" },
            { icon: "sun", label: "Sattvic Diet" },
            { icon: "shield", label: "Detox Food" },
            { icon: "heart", label: "Ayurvedic" },
          ],
    
          philosophy: [
            { icon: "leaf", title: "Sattvic", desc: "Pure detox food" },
            { icon: "globe", title: "Ayurvedic", desc: "Dosha balancing" },
            { icon: "heart", title: "Healing", desc: "Nourishes renewal" },
            { icon: "feather", title: "Light", desc: "Supports cleansing" },
          ],
        },
      },
    excursion: {
        colors: {
          white: "#ffffff",
          navy: "#1A2E10",
          sage: "#4A7A30",
          overlay:
            "linear-gradient(to top, rgba(0,0,0,0.6), transparent)",
          cardShadow: "0 8px 24px rgba(0,0,0,0.1)",
        },
    
        content: {
          eyebrow: "Retreat Experiences",
          title: "Explore",
          highlight: "Bali's Healing Beauty",
          subtitle:
            "Bali's tropical climate, peaceful nature, great culture and spiritual atmosphere help balance your training and relaxation.",
    
          excursions: [
            {
              title: "Tirta Empul Temple",
              tag: "Spiritual",
              tagBg: "#4A7A30",
              duration: "Half Day",
              icon: "globe",
              img: "https://wallpaperaccess.com/full/139108.jpg",
              desc: "Sacred purification temple — perfect complement to your inner cleansing retreat.",
              highlights: ["Holy water ritual", "Sacred energy", "Spiritual cleansing"],
            },
            {
              title: "Tegallalang Rice Terraces",
              tag: "Nature",
              tagBg: "#7AAA5A",
              duration: "Half Day",
              icon: "leaf",
              img: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80",
              desc: "Serene green rice terraces — ideal for mindful walking and reconnection with nature.",
              highlights: ["Peaceful walks", "Scenic views", "Nature therapy"],
            },
            {
              title: "Monkey Forest",
              tag: "Nature",
              tagBg: "#6A8A30",
              duration: "2 Hours",
              icon: "trees",
              img: "https://images.unsplash.com/photo-1567745576352-e404ee640705?w=800&q=80",
              desc: "Lush jungle sanctuary with ancient temples and playful wild monkeys.",
              highlights: ["Wild monkeys", "Temple ruins", "Jungle walk"],
            },
            {
              title: "Mount Batur",
              tag: "Adventure",
              tagBg: "#8B4A2A",
              duration: "Full Day",
              icon: "mountain",
              img: "https://images.unsplash.com/photo-1555400038-63f5ba517a47?w=800&q=80",
              desc: "Sunrise volcano trek — energizing and grounding for your retreat journey.",
              highlights: ["Sunrise energy", "Volcano hike", "Lake views"],
            },
            {
              title: "Tanah Lot Temple",
              tag: "Cultural",
              tagBg: "#1A2E10",
              duration: "Half Day",
              icon: "waves",
              img: "https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=800&q=80",
              desc: "Iconic ocean temple with breathtaking sunset — deeply calming for retreat guests.",
              highlights: ["Sunset healing", "Ocean energy", "Cultural depth"],
            },
            {
              title: "Ubud Cafes & Markets",
              tag: "Free Time",
              tagBg: "#5A7A3A",
              duration: "Flexible",
              icon: "camera",
              img: "https://wallpaperaccess.com/full/139108.jpg",
              desc: "Explore Ubud's cafes for authentic Balinese food and local markets on free afternoons.",
              highlights: ["Local cuisine", "Art & craft", "Self-discovery"],
            },
          ],
    
          bottom: {
            title: "Free time to explore beautiful Bali ✨",
            desc: "Afternoons and Sundays free to explore cafes, temples and nature",
          },
        },
      },
    location: {
        colors: {
          cream: "#F5F8F0",
          navy: "#1A2E10",
          violet: "#4A7A30",
          white: "#ffffff",
          activeBg: "rgba(74,122,48,0.1)",
          border: "rgba(0,0,0,0.05)",
        },
    
        content: {
          eyebrow: "Retreat Location",
          title: "Spiritual",
          highlight: "heart of Bali",
          subtitle:
            "Located in Ubud — Bali's most healing destination — tropical climate, peaceful nature, great culture and spiritual atmosphere.",
    
          address: "No 77 A, Jl. Bisma, Ubud, Kecamatan Ubud, Kabupaten Gianyar, Bali 80571, Indonesia",
    
          mapEmbed:
            "https://www.google.com/maps?q=Jl.+Bisma+Ubud+Bali+Indonesia&output=embed",
    
          spots: [
            {
              name: "Ubud Center",
              distance: "5 min walk",
              desc: "Heart of Bali's culture with cafés, markets & temples.",
              icon: "globe",
            },
            {
              name: "Rice Terraces",
              distance: "15 min drive",
              desc: "Famous green landscapes for mindful walking.",
              icon: "leaf",
            },
            {
              name: "Tirta Empul",
              distance: "20 min drive",
              desc: "Sacred water temple for purification rituals.",
              icon: "sparkles",
            },
            {
              name: "Monkey Forest",
              distance: "8 min walk",
              desc: "Jungle sanctuary with ancient temples.",
              icon: "trees",
            },
            {
              name: "Mount Batur",
              distance: "45 min drive",
              desc: "Sunrise trekking and grounding volcanic energy.",
              icon: "mountain",
            },
          ],
    
          stats: [
            { label: "Airport", value: "~75 min", icon: "globe" },
            { label: "Town", value: "5 min", icon: "map" },
            { label: "Altitude", value: "450m", icon: "mountain" },
            { label: "Climate", value: "Tropical", icon: "sun" },
          ],
        },
      },
    massage: {
      colors: {
        navy: "#1A2456",
        white: "#ffffff",
        sage: "#7BAF8A",
        border: "rgba(255,255,255,0.15)",
        cardBg: "rgba(255, 255, 255, 0.05)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      },
      content: {
        title: "Signature",
        highlight: "Massage Treatments",
        subtitle: "Unwind your body and restore your muscle flexibility after intensive yoga training sessions with expert therapies.",
        massages: [
          {
            id: 1,
            title: "Deep Tissue Ritual",
            tagline: "Designed to relieve deep-seated muscle tension, chronic stress, and stiffness using slow, deliberate strokes.",
            duration: "60 mins",
            rating: 4.9,
            reviews: 184,
            isPopular: true,
            isPaid: true,
            image: "https://images.unsplash.com/photo-1600334129128-685c5582fd35?auto=format&fit=crop&w=600&q=80"
          },
          {
            id: 2,
            title: "Himalayan Hot Stone",
            tagline: "Warm, mineral-rich basalt stones are placed strategically to melt away anxiety and restore deeply aligned energy flow.",
            duration: "75 mins",
            rating: 4.8,
            reviews: 92,
            isPopular: false,
            isPaid: true,
            image: "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&w=600&q=80"
          },
          {
            id: 3,
            title: "Complimentary Aroma Mist",
            tagline: "A brief, relaxing hydration aromatherapy session available to all resort guests during morning welcome check-ins.",
            duration: "15 mins",
            rating: 5.0,
            reviews: 210,
            isPopular: false,
            isPaid: false,
            image: "https://images.unsplash.com/photo-1519699047748-de8e457a634e?auto=format&fit=crop&w=600&q=80"
          },
          {
            id: 4,
            title: "Swedish Bliss Massage",
            tagline: "Long, gliding strokes mixed with kneading to promote total body relaxation, improved circulation, and optimal flexibility.",
            duration: "50 mins",
            rating: 4.7,
            reviews: 145,
            isPopular: false,
            isPaid: true,
            image: "https://images.unsplash.com/photo-1515377905703-c4788e51af15?auto=format&fit=crop&w=600&q=80"
          },
          {
            id: 5,
            title: "Welcome Scalp Massage",
            tagline: "Enjoy a quick, stress-relieving tension release head massage routine. Completely complimentary.",
            duration: "10 mins",
            rating: 4.9,
            reviews: 76,
            isPopular: true,
            isPaid: false,
            image: "https://images.unsplash.com/photo-1540555700478-4be289fbecef?auto=format&fit=crop&w=600&q=80"
          }
        ]
      }
    },
  },

  accommodationSection: {
      colors: {
        navy: "#1A2E10",
        white: "#ffffff",
        sage: "#4A7A30",
        border: "rgba(255,255,255,0.25)",
        cardBg: "rgba(255,255,255,0.08)",
        shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      },
  
      content: {
        title: "Your",
        highlight: "Retreat Stay in Bali",
        subtitle: "Choose from two accommodation options for your 3-day wellness retreat.",
        rooms: [
        {
          type: "2 Sharing Room",
          tag: "Included",
          tagBg: "#4A7A30",
          price: "$350",
          img: "https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=1200&q=80",
          desc: "Comfortable shared room with sattvic meals and wellness therapy included.",
          features: [
            "Twin sharing",
            "Sattvic meals included",
            "WiFi",
            "Daily cleaning",
          ],
        },
        {
          type: "Private Room",
          tag: "Upgrade",
          tagBg: "#2E5018",
          price: "$400",
          img: "https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=1200&q=80",
          desc: "Private room with sattvic meals and wellness therapy included.",
          features: [
            "Private room",
            "Sattvic meals included",
            "WiFi",
            "Daily cleaning",
          ],
        },
      ],
      amenities: [
          { icon: "leaf", label: "Garden" },
          { icon: "wifi", label: "WiFi" },
          { icon: "feather", label: "Yoga Hall" },
          { icon: "heart", label: "Ayurvedic Therapy" },
        ],
        buttonText: "Book Your Retreat",
        url: "/contact",
      },
    },

  faqSection: {
      colors: {
        navy: "#1A2E10",
        white: "#ffffff",
        sage: "#4A7A30",
        border: "rgba(255,255,255,0.25)",
        cardBg: "rgba(255,255,255,0.08)",
        shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      },
  
      content: {
        title: "Wellness Retreat",
        highlight: "FAQ",
        subtitle: "Find answers to the most common questions about the 3-Day Yoga and Wellness Retreat in Bali.",
        faqs: [
          {
            question: "What is the 3-Day Yoga & Wellness Retreat in Bali?",
            answer:
              "A compact, impactful retreat combining basic asanas, pranayama, meditation, yogic philosophy, and Panchakarma therapy with a personalized Ayurvedic Doctor consultation.",
          },
          {
            question: "Who can enroll in this program?",
            answer: "Working professionals and anyone seeking a short, meaningful wellness break can enroll.",
          },
          {
            question: "Do I need any experience before entering this course?",
            answer: "No prior experience is needed; the program teaches fundamental yoga principles from the start.",
          },
          {
            question: "Will we be certified on course completion?",
            answer: "Please contact us for current certification details for this specific program.",
          },
          {
            question: "What kind of housing is offered?",
            answer: "2 Sharing Room and Private Room options are available.",
          },
          {
            question: "What kind of food do you serve over the course?",
            answer: "Freshly cooked sattvic meals are served 3 times a day.",
          },
        ],
      },
    },
};