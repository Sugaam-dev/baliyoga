import { PROGRAM_PRICES_RISHIKESH } from "../../programPricesRishikesh";

export const Rishikesh200HoursData = {
  heroSection: {
      colors: {
        cream: "#FFF8F0",
  goldLight: "#D4891A",
  navy: "#2D1A00",
  sage: "#8A5A10",
  overlay: "linear-gradient(180deg, rgba(45,26,0,0.55), rgba(45,26,0,0.9))",
        white: "#ffffff",
        textLight: "rgba(255,255,255,0.85)",
        textFade: "rgba(255,255,255,0.8)",
      },
  
      hero: {
       location: "Rishikesh, India · 200-Hour Professional",
  title: "Multi-Style YTTC",
  highlight: "20 Days 200 Hours ",
  subtitle: "200-hour yoga teacher training in Rishikesh — Hatha, Ashtanga, RYT-200 & Himalayan tradition",
  price: PROGRAM_PRICES_RISHIKESH.multiStyle["200hr"],
        priceNote: "Includes stay, meals & certification",
        bgImage:
          "/images/external/general/329294-magnificent-yoga-wallpaper-ralph-smart-infinite-waters-diving-deep.webp",
        buttonText: "Book Now",
          url:"/contact",
      },
  
      gains: [
       "Yoga Alliance RYT-200 globally recognized certification",
  "Complete Hatha & Ashtanga Vinyasa yoga — 100+ postures with deep understanding",
  "Pranayama, Yoga Nidra, meditation, mantra, mudra & Bandha practices",
  "Yoga therapy, anatomy, alignment, adjustment & teaching methodology",
  "Extra classes on Ayurveda, Kundalini, Tantra & Therapy",
  "Evening Ganga beach meditation, fire ceremony & Himalayan excursions",
      ],
  
      certificates: [
        { img: "/images/cirtificats/200.webp", label: "200 Hrs" },
        { img: "/images/cirtificats/yoga.webp", label: "Certified" },
        { img: "/images/cirtificats/YACEP.webp", label: "Yoga Alliance" }
      ],
  
      content: {
        bottomText:
         "Receive a Yoga Alliance RYT-200 certification and begin your professional teaching career from the birthplace of yoga."
      },
    },

  highlightsSection: {
    community: {
        colors: {
          white: "#ffffff",
          goldLight: "#C8874A",
         overlay: "linear-gradient(180deg, rgba(80,45,0,0.65), rgba(35,18,0,0.95))",
    
          textLight: "rgba(255,255,255,0.75)",
          textFade: "rgba(255,255,255,0.7)",
          glass: "rgba(255,255,255,0.08)",
          glassHover: "rgba(255,255,255,0.15)",
          border: "rgba(255,255,255,0.2)",
        },
    
        content: {
          bgImage:
            "/images/external/general/329294-magnificent-yoga-wallpaper-ralph-smart-infinite-waters-diving-deep.webp",
      eyebrow: "Ombreathe Experience",
    title: "Join our",
    highlight: "transformative",
    subtitle: "Experience authentic yoga in Rishikesh with Himalayan masters, deep Vedic philosophy and Yoga Alliance RYT-200 certification.",
    
            stats: [
            { value: "200+", label: "Training Hours", icon: "clock" },
    
      { value: "25 Days", label: "Immersive Journey", icon: "calendar" },
      { value: "RYT-200", label: "Yoga Alliance Cert", icon: "award" },
            { value: "15,000+", label: "Teachers Trained", icon: "award" },
            { value: "121+", label: "Countries Represented", icon: "globe" },
          ],
         bottomText1: "From India's biggest yoga school — guided by Yogi Vishnu with 35 years of experience in the ancient Himalayan tradition.",
    bottomText2: "One month of training and lifetime learning — arrive 1st, depart 25th of each month in Rishikesh.",
        },
      },
    promo: {
        colors: {
        navy: "#2D1A00",
    violet: "#8A5A10",
          white: "#ffffff",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
          overlay: "linear-gradient(to top, rgba(0,0,0,0.4), transparent)",
        },
    
        content: {
          eyebrow: "Rishikesh Yoga Experience",
          title: "Join our",
          highlight: "immersive",
          duration: "25-day",
          strongText: "Rishikesh",
          features: [
            {
              icon: "award",
              title: "200-Hour Certificate",
              sub: "Eligible for Yoga Alliance RYT-200 pathway",
            },
            {
              icon: "bed",
              title: "Himalayan Accommodation",
              sub: "Fully furnished rooms with private bathrooms",
            },
            {
              icon: "utensils",
              title: "Organic Vegetarian Meals",
              sub: "Traditional Ayurvedic Indian diet daily",
            },
            {
              icon: "heart",
              title: "Ganga Beach Meditation",
              sub: "Evening meditation on the sacred Ganges",
            },
            {
              icon: "compass",
              title: "Himalayan Excursions",
              sub: "Kunjapuri Temple, Beatles Ashram & more",
            },
          ],
          images: {
            main:
              "/images/external/heroes/499607-yoga-background.webp",
            food:
              "/images/external/general/unsplash_photo-1512621776951-a57141f2eefd.webp",
            stay:
              "/images/external/general/unsplash_photo-1626621341517-bbf3d9990a23.webp",
          },
        },
      }

  },

  practiceSection: {
      colors: {
        navy: "#1E2D0E",
        cream: "#F5EFE8",
        white: "#ffffff",
        shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
        overlay: "linear-gradient(to top, rgba(0,0,0,0.4), transparent)",
      },
  
      content: {
        eyebrow: "Core Practices",
      title: "Ancient Traditions",
        highlight: "Hatha, Ashtanga & Himalayan Yoga",
        subtitle: "Build a complete professional yoga foundation — Hatha, Ashtanga, Pranayama, Yoga Therapy and Himalayan wisdom for RYT-200.",
  
        practices: [
          {
            label: "Hatha & Ashtanga Yoga",
            desc: "200+ postures — traditional Hatha yoga and Ashtanga practice with deep alignment and adjustment",
            icon: "sun",
            img: "/images/external/heroes/yoga-background-2rmnmb16t7q2lj98.webp",
          },
          {
            label: "Pranayama & Meditation",
            desc: "Yoga Sadhana, breathing techniques, Yoga Nidra, Bandha, Mudra & evening Ganga beach meditation",
            icon: "wind",
            img: "/images/external/general/unsplash_photo-1593811167562-9cef47bfc4d7.webp",
          },
          {
            label: "Philosophy & Teaching",
            desc: "Mantra chanting, yoga philosophy, anatomy, alignment & teaching methodology from Himalayan masters",
            icon: "feather",
            img: "/images/external/rooms/unsplash_photo-1604881991720-f91add269bed.webp",
          },
        ],
        specialTitle: "Why Choose",
        specialHighlight: "This Training",
        specials: [
          {
            title: "India's Biggest Yoga School",
            desc: "15,000+ teachers trained from 121+ countries — guided by Yogi Vishnu with 35 years of experience",
            icon: "heart",
          },
          {
            title: "Himalayan Tradition",
            desc: "5,000-year-old yoga tradition from Agastya Muni to Patanjali, Mahavatar Babaji & Guru Vishnu",
            icon: "shield",
          },
          {
            title: "Sacred Location",
            desc: "On the banks of the Ganges, 200m from the river — Ganga meditation, forest & Himalayan views",
            icon: "sparkles",
          },
        ],
      },
    },

  programDetailsSection: {
    curriculum: {
        colors: {
         navy: "#2D1A00",
    violet: "#8A5A10",
    sage: "#C8891A",
          cream: "#F5EFE8",
          white: "#ffffff",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      gradientCard: "linear-gradient(135deg,#2D1A00,#8A5A10)",
    gradientCTA: "linear-gradient(135deg,#2D1A00,#5A3A00)",
        },
    
        content: {
          eyebrow: "Curriculum",
          title: "Your Complete",
         highlight: "200-Hour Journey",
    subtitle: "A comprehensive program covering Mantra, Hatha, Ashtanga, Pranayama, Yoga Therapy, Bandha and complete teaching methodology.",
    
          courses: [
            {
              title: "Hatha Yoga",
              icon: "sun",
              items: [
                "200+ yoga postures",
                "Sun salutations",
                "Alignment basics",
                "Yoga cleansing",
              ],
            },
            {
              title: "Ashtanga Yoga",
              icon: "dumbbell",
              items: [
                "Primary series",
                "Dynamic sequences",
                "Strength & flexibility",
                "Breath synchronization",
              ],
            },
            {
      title: "Bandha & Energy",
      icon: "sparkles",
      items: [
        "Bandha energy locks",
        "Mudra practice",
        "Energy channeling",
        "Kundalini basics",
      ],
    },
            {
              title: "Pranayama",
              icon: "wind",
              items: [
                "Yoga Sadhana",
                "Breathing techniques",
                "Bandha & Mudra",
                "Energy balance",
              ],
            },
            {
              title: "Yoga Philosophy",
              icon: "book",
              items: [
                "Himalayan tradition",
                "Mantra & chanting",
                "Yoga lifestyle",
                "Vedic wisdom",
              ],
            },
            {
              title: "Anatomy & Physiology",
              icon: "heart",
              items: [
                "Body awareness",
                "Spine & joint care",
                "Injury prevention",
                "Yogic science",
              ],
            },
            {
              title: "Meditation & Yoga Nidra",
              icon: "brain",
              items: [
                "Yoga Sadhana",
                "Yoga Nidra",
                "Ganga beach meditation",
                "Inner awareness",
              ],
            },
         {
      title: "Yoga Therapy",
      icon: "heart",
      items: [
        "Therapeutic applications",
        "Healing through yoga",
        "Ayurvedic principles",
        "Body-mind wellness",
      ],
    },
            {
              title: "Teaching Methodology",
              icon: "users",
              items: [
                "Class structure",
                "Cueing techniques",
                "Mindfulness training",
                "Student guidance",
              ],
            },
          ],
    
          phases: [
     { title: "Foundation", days: "Days 1–8",  desc: "Yoga Sadhana, Pranayama, Hatha yoga & philosophy introduction",  color: "#5A3A00" },
    { title: "Development", days: "Days 9–17", desc: "Ashtanga, anatomy, Yoga Therapy, Bandha & teaching methodology",  color: "#8A5A10" },
    { title: "Integration",  days: "Days 18–25", desc: "Teaching practice, Yoga Nidra, Ganga meditation & certification", color: "#C8891A" },
    
          ],
    
          cta: {
          title: "Become a Certified RYT-200 Yoga Teacher",
    desc: "Transform your body, mind, and life through 200-hour yoga teacher training in the birthplace of yoga.",
    
            buttonText: "Start Your Journey",
              url:"/contact",
          },
        },
      },
    schedule: {
        colors: {
          cream: "#F5EFE8",
          creamDark: "#EAE0D0",
          navy: "#1E2D0E",
          violet: "#4A7A2A",
          white: "#ffffff",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
          noteBg: "rgba(138,90,16,0.08)",
        },
    
        content: {
          eyebrow: "Daily Schedule",
          title: "Daily Flow of",
          highlight: "Training",
          subtitle: "A structured daily routine from early morning Yoga Sadhana to evening Ganga beach meditation — all classes are mandatory.",
    
          image:
            "/images/external/heroes/499607-yoga-background.webp",
    
          morning: [
            { time: "05:00 AM", activity: "Yoga Sadhana", icon: "moon" },
            { time: "06:15 AM", activity: "Pranayama & Meditation", icon: "wind" },
         { time: "07:30 AM", activity: "Ashtanga Yoga", icon: "dumbbell" },
            { time: "09:00 AM", activity: "Breakfast", icon: "coffee" },
            { time: "10:45 AM", activity: "Yoga Anatomy", icon: "brain" },
            { time: "12:00 PM", activity: "Alignment & Adjustment / Teaching Methodology", icon: "users" },
          ],
    
          afternoon: [
            { time: "01:00 PM", activity: "Lunch", icon: "utensils" },
            { time: "02:00 PM", activity: "Self Study", icon: "book" },
            { time: "03:30 PM", activity: "Yoga Philosophy", icon: "feather" },
       { time: "04:30 PM", activity: "Classical Hatha Yoga", icon: "sun" },
            { time: "06:00 PM", activity: "Dinner", icon: "utensils" },
            { time: "07:00 PM", activity: "Ganga Beach Meditation (Self)", icon: "moon" },
          ],
    
          sections: {
            morningTitle: "Morning",
            afternoonTitle: "Afternoon & Evening",
            note:
              "All classes are mandatory. Leave only in case of emergency or illness with teacher permission. Depending on season and weather, timetable may change.",
          },
        },
      },
  },

  experienceSection: {
    food: {
        colors: {
          cream: "#F5EFE8",
          navy: "#1E2D0E",
          white: "#ffffff",
          sage: "#4A7A2A",
          shadowSm: "0 4px 12px rgba(0,0,0,0.08)",
          shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
        },
    
        content: {
          eyebrow: "Food Experience",
          title: "Nourish your",
          highlight: "body & soul",
          subtitle:
            "Organic Ayurvedic vegetarian meals 3 times daily — traditional Indian yogic diet with herbal teas and filtered water.",
    
          meals: [
            {
              key: "breakfast",
              title: "Breakfast",
              icon: "sun",
              time: "09:00 – 09:45",
              img: "/images/external/foods/easy_vegetarian_indian_breakfast_recipes.webp",
              desc: "Nourishing yogic breakfast after your morning Sadhana and Hatha yoga practice.",
              items: [
                "Seasonal fresh fruits",
                "Herbal teas",
                "Traditional Indian dishes",
                "Juices & filtered water",
              ],
            },
            {
              key: "lunch",
              title: "Lunch",
              icon: "coffee",
              time: "01:00 – 02:00",
              img: "/images/external/general/unsplash_photo-1512621776951-a57141f2eefd.webp",
              desc: "Traditional Ayurvedic Indian vegetarian lunch — easy to digest and energizing.",
              items: [
                "Rice & chapatti",
                "Lentil dal",
                "Seasonal vegetable dishes",
                "Yogic diet staples",
              ],
            },
            {
              key: "dinner",
              title: "Dinner",
              icon: "moon",
              time: "06:00 – 07:00",
              img: "/images/external/general/unsplash_photo-1547592180-85f173990554.webp",
              desc: "Light sattvic evening meals before Ganga beach meditation — Sundays explore local Rishikesh restaurants.",
              items: [
                "Lentil soups",
                "Vegetable dishes",
                "Chapatti & rice",
                "Herbal teas",
              ],
            },
          ],
    
          dietary: [
            { icon: "leaf", label: "Vegetarian" },
            { icon: "feather", label: "Vegan Options" },
            { icon: "sparkles", label: "No Meat / Egg" },
            { icon: "sun", label: "Ayurvedic" },
            { icon: "shield", label: "No Alcohol" },
            { icon: "heart", label: "Organic" },
          ],
    
          philosophy: [
            { icon: "leaf", title: "Sattvic", desc: "Traditional yogic food" },
            { icon: "globe", title: "Ayurvedic", desc: "India's ancient health science" },
            { icon: "heart", title: "Mindful", desc: "Prepared with love" },
            { icon: "feather", title: "Balanced", desc: "Easy to digest" },
          ],
        },
      },
    excursion: {
        colors: {
          white: "#ffffff",
         navy: "#2D1A00",
    sage: "#8A5A10",
          overlay:
            "linear-gradient(to top, rgba(0,0,0,0.6), transparent)",
          cardShadow: "0 8px 24px rgba(0,0,0,0.1)",
        },
    
        content: {
          eyebrow: "Excursions",
          title: "Explore",
          highlight: "Rishikesh Experiences",
          subtitle:
            "Explore the beauty of Rishikesh on 2 Sundays during the course — all excursions included in the course fee.",
    
          excursions: [
            {
              title: "Kunjapuri Temple",
              tag: "Spiritual",
              tagBg: "#4A7A2A",
              duration: "Half Day",
              icon: "mountain",
              img: "/images/external/general/unsplash_photo-1626621341517-bbf3d9990a23.webp",
              desc: "Himalayan temple with breathtaking sunrise views over the mountains.",
              highlights: ["Sunrise views", "Himalayan temple", "Spiritual energy"],
            },
            {
              title: "Beatles Ashram",
              tag: "Cultural",
              tagBg: "#C8874A",
              duration: "Half Day",
              icon: "globe",
              img: "/images/external/heroes/499607-yoga-background.webp",
              desc: "Historic ashram where The Beatles studied Transcendental Meditation in 1968.",
              highlights: ["Historic ashram", "Street art", "Meditation history"],
            },
            {
              title: "Parmarth Niketan",
              tag: "Spiritual",
              tagBg: "#1E2D0E",
              duration: "2 Hours",
              icon: "sparkles",
              img: "/images/external/general/139108.webp",
              desc: "Famous Ganga Aarti ceremony at one of India's largest ashrams.",
              highlights: ["Ganga Aarti", "Sacred ceremony", "Ashram visit"],
            },
            {
              title: "Vashishta Cave",
              tag: "Sacred",
              tagBg: "#6A4A20",
              duration: "Half Day",
              icon: "leaf",
              img: "/images/external/general/unsplash_photo-1558618666-fcd25c85cd64.webp",
              desc: "Ancient meditation cave where sage Vashishta practiced yoga.",
              highlights: ["Ancient cave", "Meditation site", "Spiritual history"],
            },
            {
              title: "Ganga Beach Yoga",
              tag: "Practice",
              tagBg: "#2E5018",
              duration: "Daily",
              icon: "waves",
              img: "/images/external/general/unsplash_photo-1544367567-0f2fcb009e0b.webp",
              desc: "Evening meditation and Kirtan on the sacred banks of the Ganges river.",
              highlights: ["Sacred Ganges", "Evening Kirtan", "Sunset meditation"],
            },
            {
              title: "Study Tours",
              tag: "Learning",
              tagBg: "#4A6A20",
              duration: "Half Day",
              icon: "book",
              img: "/images/external/general/139108.webp",
              desc: "Study tours to different ashrams and temples in and around Rishikesh.",
              highlights: ["Ashram visits", "Temple tours", "Yogic culture"],
            },
          ],
    
          bottom: {
            title: "All excursions included ✨",
            desc: "2 Sundays of excursions — transport & guides covered in course fee",
          },
        },
      },
    location: {
        colors: {
          cream: "#F5EFE8",
          navy: "#1E2D0E",
          violet: "#4A7A2A",
          white: "#ffffff",
          activeBg: "rgba(74,122,42,0.1)",
          border: "rgba(0,0,0,0.05)",
        },
    
        content: {
          eyebrow: "Location",
          title: "Yoga Capital",
          highlight: "of the World",
          subtitle:
            "Located in Rishikesh — beside the sacred Ganges, beneath the eternal Himalayas — 15+ years of authentic yoga tradition.",
    
          address: "Ombreathe, Police Station, behind Laxman Jhula, Laxman Jhula, Rishikesh, Uttarakhand 249302",
    
          mapEmbed:
            "https://www.google.com/maps?q=Rishikesh&output=embed",
    
          spots: [
            {
              name: "Laxman Jhula",
              distance: "2 min walk",
              desc: "Iconic suspension bridge over the sacred Ganges river.",
              icon: "waves",
            },
            {
              name: "Ganga River",
              distance: "200m walk",
              desc: "Sacred river for evening meditation, Kirtan and Aarti.",
              icon: "sparkles",
            },
            {
              name: "Kunjapuri Temple",
              distance: "45 min drive",
              desc: "Himalayan temple with panoramic sunrise views.",
              icon: "mountain",
            },
            {
              name: "Beatles Ashram",
              distance: "20 min walk",
              desc: "Historic ashram where The Beatles studied yoga in 1968.",
              icon: "globe",
            },
            {
              name: "Parmarth Niketan",
              distance: "15 min walk",
              desc: "India's largest ashram — famous Ganga Aarti ceremony.",
              icon: "leaf",
            },
          ],
    
          stats: [
            { label: "Airport", value: "Dehradun 30 min", icon: "globe" },
            { label: "Delhi", value: "6 hrs by car", icon: "map" },
            { label: "Altitude", value: "372m", icon: "mountain" },
            { label: "Climate", value: "Himalayan", icon: "sun" },
          ],
        },
      },
    massage: {
      colors: {
        navy: "#1A2456",
        white: "#ffffff",
        sage: "#7BAF8A",
        border: "rgba(255,255,255,0.15)",
        cardBg: "rgba(255, 255, 255, 0.05)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      },
      content: {
        title: "Signature",
        highlight: "Massage Treatments",
        subtitle: "Unwind your body and restore your muscle flexibility after intensive yoga training sessions with expert therapies.",
        massages: [
          {
            id: 1,
            title: "Deep Tissue Ritual",
            tagline: "Designed to relieve deep-seated muscle tension, chronic stress, and stiffness using slow, deliberate strokes.",
            duration: "60 mins",
            rating: 4.9,
            reviews: 184,
            isPopular: true,
            isPaid: true,
            image: "/images/external/general/unsplash_photo-1600334129128-685c5582fd35.webp",
          },
          {
            id: 2,
            title: "Himalayan Hot Stone",
            tagline: "Warm, mineral-rich basalt stones are placed strategically to melt away anxiety and restore deeply aligned energy flow.",
            duration: "75 mins",
            rating: 4.8,
            reviews: 92,
            isPopular: false,
            isPaid: true,
            image: "/images/external/general/unsplash_photo-1544161515-4ab6ce6db874.webp",
          },
          {
            id: 3,
            title: "Complimentary Aroma Mist",
            tagline: "A brief, relaxing hydration aromatherapy session available to all resort guests during morning welcome check-ins.",
            duration: "15 mins",
            rating: 5.0,
            reviews: 210,
            isPopular: false,
            isPaid: false,
            image: "/images/external/general/unsplash_photo-1519699047748-de8e457a634e.webp",
          },
          {
            id: 4,
            title: "Swedish Bliss Massage",
            tagline: "Long, gliding strokes mixed with kneading to promote total body relaxation, improved circulation, and optimal flexibility.",
            duration: "50 mins",
            rating: 4.7,
            reviews: 145,
            isPopular: false,
            isPaid: true,
            image: "/images/external/general/unsplash_photo-1515377905703-c4788e51af15.webp",
          },
          {
            id: 5,
            title: "Welcome Scalp Massage",
            tagline: "Enjoy a quick, stress-relieving tension release head massage routine. Completely complimentary.",
            duration: "10 mins",
            rating: 4.9,
            reviews: 76,
            isPopular: true,
            isPaid: false,
            image: "/images/external/general/unsplash_photo-1540555700478-4be289fbecef.webp",
          }
        ]
      }
    },
  },

  accommodationSection: {
      colors: {
        white: "#ffffff",
       navy: "#2D1A00",
  sage: "#8A5A10",
        border: "rgba(255,255,255,0.25)",
        cardBg: "rgba(255,255,255,0.08)",
        shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      },
  
      content: {
        title: "Your",
        highlight: "Stay in Rishikesh",
     subtitle: "Fully furnished rooms with private bathrooms — beside the Himalayan forest, Ganga river 200m away, for your 25-day journey.",
  
        rooms: [
        {
          type: "6 Sharing Room",
          tag: "Budget",
          tagBg: "#7BAF8A",
          price: "$999",
          img: "/images/external/general/unsplash_photo-1505691938895-1758d7feb511.webp",
          desc: "Comfortable 6-bed shared rooms with AC, washroom and modern amenities.",
          features: [
            "6-bed sharing",
            "Air conditioning",
            "Shared washroom",
            "WiFi",
            "Clean towels & bed sheets",
            "Daily cleaning",
          ],
        },
        {
          type: "4 Sharing Room",
          tag: "Comfort",
          tagBg: "#E5A93B",
          price: "$1,099",
          img: "/images/external/general/unsplash_photo-1555854877-bab0e564b8d5.webp",
          desc: "Spacious 4-bed shared rooms with AC, washroom and modern conveniences.",
          features: [
            "4-bed sharing",
            "Air conditioning",
            "Shared washroom",
            "WiFi",
            "Clean towels & bed sheets",
            "Daily cleaning",
          ],
        },
        {
          type: "2 Sharing Room",
          tag: "Popular",
          tagBg: "#2E5018",
          price: "$1,399",
          img: "/images/external/general/unsplash_photo-1631049307264-da0ec9d70304.webp",
          desc: "Twin sharing room with private bathroom, wardrobe, study desk and clean comfortable environment.",
          features: [
            "Twin sharing",
            "Private bathroom",
            "Study desk",
            "Wardrobe",
            "WiFi",
            "Towels provided",
          ],
        },
        {
          type: "Private Room",
          tag: "Upgrade",
          tagBg: "#C8874A",
          price: "$1,699",
          img: "/images/external/general/unsplash_photo-1618773928121-c32242e63f39.webp",
          desc: "Private room with private bathroom, all modern amenities and peaceful Himalayan ambience.",
          features: [
            "Private room",
            "Private bathroom",
            "Study desk",
            "Wardrobe",
            "WiFi",
            "AC available (+$150)",
          ],
        },
      ],
      amenities: [
          { icon: "trees", label: "Himalayan Forest" },
          { icon: "waves", label: "Ganga — 200m" },
          { icon: "heart", label: "Meditation Cave" },
          { icon: "wifi", label: "Free WiFi" },
          { icon: "utensils", label: "Dining Hall" },
          { icon: "feather", label: "3 Yoga Halls" },
          { icon: "leaf", label: "Garden" },
          { icon: "book", label: "Library" },
        ],
  
        buttonText: "Book Now",
          url:"/contact",
      },
    },

  faqSection: {
      colors: {
        navy: "#1E2D0E",
        white: "#ffffff",
        sage: "#4A7A2A",
        border: "rgba(255,255,255,0.25)",
        cardBg: "rgba(255,255,255,0.08)",
        shadowMd: "0 8px 24px rgba(0,0,0,0.15)",
        shadowLg: "0 10px 30px rgba(0,0,0,0.2)",
      },
  
      content: {
        title: "Teacher",
  highlight: "Training FAQ",
  subtitle: "Find answers to the most common questions about Ombreathe's 200-hour yoga teacher training in Rishikesh.",
  
  // Replace ALL faqs with these 10:
  faqs: [
    {
      question: "Can you tell me about my room and facilities?",
      answer: "We offer clean, light and airy accommodation encased by the Himalayan mountains with the Ganga on our doorstep. All rooms have private bathrooms, free WiFi, study desk, wardrobe, fresh towels and clean bedding changed weekly. Filtered water is provided throughout. AC is available at an additional $150. Low-cost laundry service available.",
    },
    {
      question: "What food is provided?",
      answer: "You receive 3 organic vegetarian meals per day — a traditional Indian Ayurvedic diet of rice, chapatti, lentils and seasonal vegetable dishes, plus herbal tea and unlimited filtered water. WPYS food is nutritious, easy to digest and optimizes mind-body wellness.",
    },
    {
      question: "Do I need to bring any special equipment or clothes?",
      answer: "Appropriate dress code should be followed during yoga classes. One white outfit is provided for ceremony days. All yoga equipment — mats, blocks and props — is available. Please bring a water bottle. When visiting temples and the Ganga, dress modestly in line with Indian culture.",
    },
    {
      question: "What is the refund procedure?",
      answer: "The advance course fee is non-refundable. In case of emergency, you can contact us to delay the course start. Cancellations are accepted but the advance deposit will not be refunded. Once you are in the school, fees are 100% non-refundable. The course can be deferred should you need to leave early.",
    },
    {
      question: "What are the advantages of studying yoga in India?",
      answer: "India offers a perfect distraction-free environment for learning yoga where everyone has the same goal. You get to experience authentic yoga that has not altered its original form, understand yoga philosophy as a lifestyle — not just exercise — and learn directly from masters who follow the ancient Himalayan tradition.",
    },
    {
      question: "What is 200-hour yoga teacher training?",
      answer: "A 200-hour Yoga Teacher Training is the most common yoga teacher certification course. It covers every fundamental concept of yoga — anatomy, philosophy, asana, pranayama, meditation and teaching methodology — and prepares you to start teaching yoga professionally as a Yoga Alliance registered RYT-200 teacher.",
    },
    {
      question: "Can a beginner do the 200-hour yoga teacher training?",
      answer: "Yes — the 200-hour YTTC is a beginner to intermediate level course and complete beginners can join. The course is designed to meet all individual needs. Our teachers ensure students receive full attention in class, proper help in utilizing props and the freedom to work at their own speed.",
    },
    {
      question: "Can I be a certified yoga teacher with 200-hour yoga certification?",
      answer: "Yes — upon completing the 200-hour course from a Yoga Alliance certified school like Ombreathe, you will be recognized as an internationally certified yoga teacher eligible to register as RYT-200 with Yoga Alliance. This gives you an international identity to teach yoga anywhere in the world.",
    },
    {
      question: "What are the benefits of doing 200-hour yoga teacher training?",
      answer: "The training teaches advanced techniques not available in regular classes. Your body becomes stronger, your mind-body connection deepens, and you gain a thorough understanding of yoga anatomy, philosophy and teaching skills. One month of training leads to lifetime learning and the ability to teach confidently worldwide.",
    },
    {
      question: "What next after completing the 200-hour YTTC?",
      answer: "You can start teaching yoga, take advanced courses like 300-hour YTTC, deepen your practice, attend workshops, or switch styles — for example from multi-style to Kundalini. Ombreathe will always be your second home and you can extend your stay to explore Rishikesh and the Himalayas.",
    },
  ],
      },
    },
};