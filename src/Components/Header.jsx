    ...
    <header ref={navRef} className="sticky top-0 w-full bg-white shadow-md border-b border-gray-100 z-[1000]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className={`fixed inset-y-0 left-0 w-full max-w-sm bg-white shadow-2xl z-[1005] flex flex-col transition-transform duration-300 transform lg:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 flex items-center justify-between border-b border-gray-100 bg-white">
          ...
          <button 
            onClick={closeMenu}
            className="w-10 h-10 border border-gray-200 rounded-md flex items-center justify-center p-1.5 transition-colors hover:bg-gray-50"
            aria-label="Close menu"
          >
            <svg className="w-full h-full text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        </div>
                ...
                <li key={item.label} className="border-b border-gray-50 pb-2">
                  {isExpandable ? (
                    <div>
                      <button
                        onClick={() => setMobileExpandedSection(isSectionOpen ? null : item.type)}
                        className="w-full flex justify-between items-center py-3 px-3 text-gray-800 font-medium hover:bg-gray-50 rounded-lg text-left text-[16px]"
                      >
                        <span>{item.label}</span>
                        <svg className={`w-4 h-4 transition-transform duration-200 ${isSectionOpen ? 'rotate-180 text-black' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" />
                        </svg>
                      </button>
                      
                      <div className={`overflow-hidden transition-all duration-300 max-h-0 ${isSectionOpen ? 'max-h-[1200px] mt-2' : ''}`}>
                        <div className="pl-4 space-y-4 border-l border-gray-100 ml-3">
                          {MENU_CONTENT[item.type].columns.map((col, colIdx) => (
                            <div key={colIdx} className="space-y-1">
                              <span className="text-[12px] font-bold text-gray-400 uppercase tracking-wider block mb-1">{col.category}</span>
                              {col.items.map((subItem, subIdx) => (
                                <Link
                                  ...
                                  to={subItem.link}
                                  onClick={closeMenu}
                                  className="block py-1.5 px-2 text-[15px] text-gray-600 rounded hover:bg-gray-50"
                                >
                                  {subItem.name}
                                </Link>
                              ))}
                    ...
                    <Link
                      to={item.path}
                      onClick={closeMenu}
                      className="block py-3 px-3 text-gray-800 font-medium hover:bg-gray-50 rounded-lg text-[16px]"
                    >
                      {item.label}
                    </Link>
                  )}
        ...
        <div className="p-6 border-t border-gray-100 bg-white">
          <a href="#apply" onClick={closeMenu} className="block w-full text-center bg-gray-800 text-white py-3 font-semibold rounded-md shadow-md text-sm">
            APPLY NOW
          </a>
        </div>
      </div>
